...
CREATE TABLE IF NOT EXISTS `mydb`.`reservation` (
  ...
  `Duration` DECIMAL NOT NULL COMMENT 'Unit is hours.',
  `X` INT NOT NULL,
  `Y` INT NOT NULL,
  `street_NAME` VARCHAR(98) NOT NULL,
  `vehicle_ID` VARCHAR(8) NOT NULL,
  `Status` ENUM('Active', 'Waiting', 'Cancelled', 'Expired') NOT NULL,
...
-- -----------------------------------------------------
-- Table `mydb`.`Res_Leeway`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`Res_Leeway` ;

CREATE TABLE IF NOT EXISTS `mydb`.`Res_Leeway` (
  `idRes_Leeway` INT NOT NULL AUTO_INCREMENT,
  `_` INT NOT NULL DEFAULT 5,
  PRIMARY KEY (`idRes_Leeway`),
  UNIQUE INDEX `idRes_Leeway_UNIQUE` (`idRes_Leeway` ASC) VISIBLE)
ENGINE = InnoDB;
...
USE `mydb`;
DROP procedure IF EXISTS `mydb`.`insertReservation`;
...
-- -----------------------------------------------------
-- procedure setReservationLeeway
-- -----------------------------------------------------

USE `mydb`;
DROP procedure IF EXISTS `mydb`.`setReservationLeeway`;

DELIMITER $$
USE `mydb`$$
CREATE PROCEDURE `setReservationLeeway` (
	IN Minutes INT
)
BEGIN
	UPDATE Res_Leeway SET `_` = Minutes WHERE 'idRes_Leeway' = 1;
END$$
...
USE `mydb`;
DROP function IF EXISTS `mydb`.`getRes`;

DELIMITER $$
USE `mydb`$$
CREATE FUNCTION `getRes` () RETURNS INT DETERMINISTIC
BEGIN
	DECLARE TMP INT;
	SELECT `_` FROM Res_Leeway WHERE `idRes_Leeway` = 1 INTO TMP;
    RETURN TMP;
END$$
...
USE `mydb`$$
DROP TRIGGER IF EXISTS `mydb`.`vehiclelog_AFTER_INSERT_1` $$
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`vehiclelog_AFTER_INSERT_1` AFTER INSERT ON `vehiclelog` FOR EACH ROW
BEGIN
DECLARE TMP INT;
SELECT `_` FROM Res_Leeway WHERE `idRes_Leeway` = 1 INTO TMP;
CREATE TEMPORARY TABLE Res_Check SELECT * FROM reservations WHERE 
	`Status` = 'Waiting' AND
	`vehicle_ID` = NEW.vehicle_ID AND 
    `street_NAME` = NEW.street_NAME AND
    `Beg_Datetime` > ADDDATE(CURRENT_TIMESTAMP, INTERVAL -TMP MINUTE) AND
    `Beg_Datetime` < ADDDATE(CURRENT_TIMESTAMP, INTERVAL TMP MINUTE) AND
    `X` >= NEW.X - 1 AND
    `X` <= NEW.X + 1 AND
    `Y` > NEW.Y - 1 AND
...
-- -----------------------------------------------------
-- Data for table `mydb`.`Res_Leeway`
-- -----------------------------------------------------
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`Res_Leeway` (`idRes_Leeway`, `_`) VALUES (DEFAULT, 5);
